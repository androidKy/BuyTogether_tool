...
import static org.proxydroid.utils.Utils.CMD_THREAD_NAME;
  ...
  private PendingIntent pendIntent;*/
    ...
    private void enableProxy() {
        ...
        try {
            ...
            if (isBypassApps) {
                // for host specified apps todo 不需要代理的APP
            ...
            if (isAutoSetProxy || isBypassApps) {
                ...
                // for host specified apps todo 添加需要代理的APP
        /*if (apps == null || apps.length <= 0)
...
//    notification.defaults |= Notification.DEFAULT_LIGHTS;
 ...
 /* private void notifyAlert(String title, String info, int flags) {
    ...
    public void onCreate() {
        super.onCreate();
        mPackageName = PreferenceManager.getDefaultSharedPreferences(this).getString(ProxyCore.KEY_PKG_NAME, "");
        BASE = "/data/data/" + getPackageName() + "/";
...
*/
    ...
    /*try {
      ...
      // Nothing
        ...
        try {
            ...
            //sendBroadcast(new Intent(ProxyCoreImpl.ACTION_PROXY_STOP));
            sendProxyStatus(false,"服务销毁，代理停止!");
        } catch (Exception e) {
    ...
    private void onDisconnect() {
        ...
        if ("https".equals(proxyType)) {
            sb.append("kill -9 `cat /data/data/" + mPackageName + "/stunnel.pid`\n");
        }

        if (isAuth && isNTLM) {
            sb.append("kill -9 `cat /data/data/" + mPackageName + "/cntlm.pid`\n");
        }

        sb.append(BASE + "proxy.sh stop\n");

        new Thread(CMD_THREAD_NAME) {
            @Override
            public void run() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_CONNECT_SUCCESS:
                    sendProxyStatus(true,"代理连接成功！");
                    //sendBroadcast(new Intent(ProxyCoreImpl.ACTION_PROXY_START));
                    ed.putBoolean("isRunning", true);
                    break;
                case MSG_CONNECT_FAIL:
                    sendProxyStatus(false,"代理连接失败！");
                    //sendBroadcast(new Intent(ProxyCoreImpl.ACTION_PROXY_STOP));
                    ed.putBoolean("isRunning", false);
                    break;
                case MSG_CONNECT_PAC_ERROR:
                    sendProxyStatus(false,"Pac连接代理失败！");
                    Toast.makeText(ProxyDroidService.this, R.string.msg_pac_error, Toast.LENGTH_SHORT)
                            .show();
                    break;
                case MSG_CONNECT_RESOLVE_ERROR:
                    sendProxyStatus(false,"域名解析失败，代理连接失败！");
    ...
    private String getProfileName() {
        ...
        return settings.getString("profile" + settings.getString("profile", "1"),
                "Profile 1" + " " + settings.getString("profile", "1"));
    }
     ...
     * @param status
     */
    private void sendProxyStatus(boolean status,String msg){
        String action = status? ProxyCoreImpl.ACTION_PROXY_START :ProxyCoreImpl.ACTION_PROXY_STOP;
    ...
    private void startProxy(Intent intent,int startId){
