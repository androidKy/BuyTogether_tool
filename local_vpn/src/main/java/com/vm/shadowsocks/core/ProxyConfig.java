   ...
   /* private String[] downloadConfig(String url) throws Exception {
    ...
    protected void loadFromLines(String[] lines) throws Exception {
        ...
        for (String line : lines) {
            LocalVpnService.Instance.writeLog("line:" + line);
            lineNumber++;
            String[] items = line.split("\\s+");
//            LocalVpnService.Instance.writeLog("items:" + Arrays.toString(items));
            if (items.length < 2) {
                continue;
            }

            String tagString = items[0].toLowerCase(Locale.ENGLISH).trim();
//            LocalVpnService.Instance.writeLog("tagString:" + tagString);
            try {
                if (!tagString.startsWith("#")) {
                    ...
                    if (tagString.equals("ip")) {
                        ...
                        LocalVpnService.Instance.writeLog("m_mtu:" + m_mtu);
                    }
                }
            } catch (Throwable e) {
