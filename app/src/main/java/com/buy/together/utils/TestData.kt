package com.buy.together.utils

/**
 * Description:
 * Created by Quinin on 2019-06-28.
 **/
class TestData {

    companion object {
        const val taskBean_str = "{\n" +
                "    \"code\":200,\n" +
                "    \"msg\":\"成功\",\n" +
                "    \"task\":{\n" +
                "        \"task_id\":1111,\n" +
                "        \"task_type\":14,\n" +
                "        \"task_status\":1,\n" +
                "        \"task_amount\":20,\n" +
                "        \"task_dispense\":5,\n" +
                "        \"task_complete\":0,\n" +
                "        \"task_fail\":0,\n" +
                "        \"time_limit\":false,\n" +
                "        \"talk_msg\":\"现在拍多久到？\",\n" +
                "        \"score_need\":true,\n" +
                "        \"comment_need\":true,\n" +
                "        \"comment_content\":\"东挑西选，头都晕了，还是这家吧，评价也不错\",\n" +
                "        \"has_commented\":false,\n" +
                "        \"create_time\":\"2019-07-23T10:17:54.282\",\n" +
                "        \"ip\":{\n" +
                "            \"city\":\"\",\n" +
                "            \"content\":\"\",\n" +
                "            \"mac_address\":\"\"\n" +
                "        },\n" +
                "        \"account\":{\n" +
                "            \"id\":22,\n" +
                "            \"user\":\"1985706049\",\n" +
                "            \"pwd\":\"sq95jqkvd91\",\n" +
                "            \"type\":0\n" +
                "        },\n" +
                "        \"goods\":{\n" +
                "            \"id\":5,\n" +
                "            \"goods_id\":1395698343,\n" +
                "            \"cat_id\":8464,\n" +
                "            \"goods_name\":\"即食柠檬片咸甜柠檬干泡茶水健胃消腻蜜饯果脯休闲零食维C柠檬果\",\n" +
                "            \"mall_name\":\"益C果城恒康美专卖店\",\n" +
                "            \"keyword\":\"即食柠檬片酸甜柠檬干维c柠檬果\",\n" +
                "            \"search_price\":\"13.8\",\n" +
                "            \"choose_info\":\"\"\n" +
                "        },\n" +
                "        \"device\":{\n" +
                "            \"id\":4220,\n" +
                "            \"bluetooth\":\"02:00:00:00:00:00\",\n" +
                "            \"imei\":\"865333035211852\",\n" +
                "            \"brand\":\"HUAWEI\",\n" +
                "            \"android\":\"8.0.0\",\n" +
                "            \"mac\":\"54:25:ea:07:4d:ae\",\n" +
                "            \"system\":\"VTR-AL00 8.0.0.342(C00)\",\n" +
                "            \"sn\":\"SJE5T17418039299\",\n" +
                "            \"imsi\":\"460006862754495\",\n" +
                "            \"model\":\"VTR-AL00\",\n" +
                "            \"useragent\":\"Dalvik/2.1.0 (Linux; U; Android 8.0.0; VTR-AL00 Build/HUAWEIVTR-AL00)\"\n" +
                "        },\n" +
                "        \"delivery_address\":{\n" +
                "            \"name\":\"邵涵育\",\n" +
                "            \"phone\":\"13319345124\",\n" +
                "            \"province\":\"广东省\",\n" +
                "            \"city\":\"佛山市\",\n" +
                "            \"district\":\"天河区\",\n" +
                "            \"street\":\"商业大厦1层\"\n" +
                "        },\n" +
                "        \"stay_time\":{\n" +
                "            \"goods_time\":4,\n" +
                "            \"search_time\":17\n" +
                "        }\n" +
                "    }\n" +
                "}"
    }
}