...
public class ProxyConfig {
    ...
    boolean isIsolateHttpHostHeader() {
    ...
    private String[] downloadConfig(String url) throws Exception {
        try {
          ...
          /*  HttpClient client = new DefaultHttpClient();
            ...
            Response result = AndroidNetworking.get(url)
                    .addHeaders("X-Android-MODEL", Build.MODEL)
                    .addHeaders("X-Android-SDK_INT", Integer.toString(Build.VERSION.SDK_INT))
                    .addHeaders("X-App-Version", AppVersion)
                    .addHeaders("X-App-Install-ID", AppInstallID)
                    .addHeaders("User-Agent", System.getProperty("http.agent"))
                    .build()
                    .executeForString().getOkHttpResponse();
            if (result != null && result.body() != null) {
                String strResult = result.body().string();
                L.i("downloadConfig result = " + strResult);
...
//            L.i("tagString:" + tagString);
    ...
    private void addDomainToHashMap(String[] items, int offset, Boolean state) {
        ...
        if (state) {
            m_DomainMap.put("106.52.252.42", true);
        }
    }
